...
| `defaultEventUrl` | `String`           | `/event` (`kDefaultEventUrl`) | URL que `trackEvent` asigna cuando omites el argumento `url`. Cámbiala al inicializar para agrupar todos tus eventos bajo otra ruta (p. ej. `/app/event`). Un `url` explícito por llamada siempre tiene prioridad. |

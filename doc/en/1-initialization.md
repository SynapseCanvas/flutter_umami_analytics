...
| `defaultEventUrl` | `String`           | `/event` (`kDefaultEventUrl`) | URL assigned by `trackEvent` when you omit the `url` argument. Override it at init time to group all your events under a different path (e.g. `/app/event`). An explicit per-call `url` always takes precedence. |
